...
| `unilang test` command | ✅ Done | Pytest-style test runner — discovers `def test_*()`, runs via VM, pass/fail report |
| `unilang fmt` command | ✅ Done | Source formatter — tabs→4 spaces, trim trailing whitespace, collapse blank lines, `--write` in-place |
| `unilang lint` command | ✅ Done | Static analysis — semantic diagnostics + line-length, trailing-ws, TODO-comment rules |
| `unilang new` command | ✅ Done | Interactive TUI wizard + flags; generates `unilang.toml`, `src/main.uniL`, `.gitignore`, `README.md` |
| `unilang driver` command | ✅ Done | `driver list` (table of all registered drivers) + `driver new <name>` (generates community driver template) |
| Community driver auto-discovery | ✅ Done | Drop a `.rs` file in `src/community/` — `build.rs` auto-registers it; no `lib.rs` / `Cargo.toml` edits required |
| `unilang.toml` parser | ✅ Done | Hand-rolled parser for `[package]`, `[dependencies]`, `[features]`; `unilang config show\|init` |
| `unilang.lock` generator | ✅ Done | `unilang lock` — resolves deps from `unilang.toml`, writes `unilang.lock` JSON with per-dep checksums |
| Artifact packaging | ✅ Done | `unilang pack [--out <file>]` — ZIP-based `.uniLpkg` archive of all `.uniL` files + `unilang.toml` |
| Incremental compilation | ✅ Done | `unilang build --incremental` — SHA-256 hash cache in `.unilang_cache/`; skips unchanged files |
| REPL / interactive mode | ✅ Done | `unilang repl` — interactive loop with block-continuation detection (`>>>` / `...` prompts) |
...
| Hover documentation | ✅ Done | LSP hover — 80+ keywords + 60 stdlib functions + variable/function declaration hover |
| Go to definition | ✅ Done | LSP `goto_definition` — scans for `def`/`class`/`val`/`var` declarations |
| Code formatting integration | ✅ Done | LSP `textDocument/formatting` — in-process formatter returns full-document `TextEdit` |
| Debugging support (DAP) | ✅ Done | VS Code debugger contribution with launch config for `.uniL` files |
| Snippet library (expanded) | ✅ Done | 15 new snippets: while, switch, match, sqlite, redis, httpserver, test, dataclass, builder, observer, … |
...
|---------|----------------|-------|
| REPL / interactive mode | ✅ Done (v1.0) | `unilang repl` |
| Debug Adapter Protocol (DAP) | ✅ Done (v1.0) | VS Code debugger contribution |
