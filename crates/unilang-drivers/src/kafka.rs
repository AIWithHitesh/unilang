...
impl UniLangDriver for KafkaDriver {
    ...
    fn register(&self, vm: &mut VM) {
        ...
        vm.register_builtin("kafka_connect", |_args| Ok(RuntimeValue::Bool(true)));
        ...
        // kafka_events(topic?) — returns List[Dict{topic,key,payload}]
        {
            ...
            vm.register_builtin("kafka_events", move |args| {
                ...
                let events: Vec<RuntimeValue> = guard
                    .iter()
                    .filter(|(t, _, _)| filter.as_ref().map_or(true, |f| f == t))
                    .map(|(t, k, p)| {
                        RuntimeValue::Dict(vec![
                            (
                                RuntimeValue::String("topic".into()),
                                RuntimeValue::String(t.clone()),
                            ),
                            (
                            ...
                            (
                                RuntimeValue::String("payload".into()),
                                RuntimeValue::String(p.clone()),
                            ),
                        ])
                    })
                    .collect();
