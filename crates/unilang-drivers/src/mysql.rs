...
impl UniLangDriver for MySqlDriver {
    ...
    fn description(&self) -> &str {
        "MySQL / MariaDB via pure-Rust mysql crate"
    }
            ...
            vm.register_builtin("mysql_query", move |args| {
                ...
                let result: Vec<RuntimeValue> = rows
                    ...
                    .map(|row| {
                        ...
                        let dict: Vec<(RuntimeValue, RuntimeValue)> = columns
                            .iter()
                            .enumerate()
                            .map(|(i, col)| {
...
fn mysql_val_to_runtime(v: &mysql::Value) -> RuntimeValue {
    match v {
        ...
        mysql::Value::Bytes(b) => RuntimeValue::String(String::from_utf8_lossy(b).into_owned()),
        mysql::Value::Date(y, mo, d, h, mi, s, _) => RuntimeValue::String(format!(
            "{:04}-{:02}-{:02} {:02}:{:02}:{:02}",
            y, mo, d, h, mi, s
        )),
        mysql::Value::Time(neg, d, h, m, s, _) => RuntimeValue::String(format!(
            "{}{}:{:02}:{:02}",
            if *neg { "-" } else { "" },
            d * 24 + *h as u32,
            m,
